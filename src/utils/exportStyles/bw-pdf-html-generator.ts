...
<head>
  ...
  --border-color: #000000;   /* Noir pur */
  --muted-color: #000000;    /* Noir pur */
}
...
/* Override toutes les couleurs CSS variables */
:root, :host, * {
...
/* SVG et icônes */
svg, [class*="icon"], [class*="Icon"], .header-stats svg {
...
/* Emojis */
[role="img"], .emoji, span[aria-label*="emoji"] {
  filter: grayscale(100%);
  opacity: 0.9;
}
...
/* CODE BLOCKS - FOND BLANC, BORDURE NOIRE */
...
/* SYNTAX HIGHLIGHTING EN NOIR UNIFORME */
.hljs-keyword,
.hljs-string,
.hljs-function,
...
@media print {
  ...
  /* Désactiver le filtre en impression si besoin */
  body, img, svg {
...
export async function generateChatHTMLForPDF_BW(  // ← _BW
  events: Event[],
